...
// Country to ISO code mapping (reused from injector)
...
const PROXY_CONFIG = {
  ...
  port: 6300,
  password: 'TPvBwkO8',
  baseUsername: '34998931-zone-custom-country'
};

/**
 * Generate a new proxy configuration for a specific country
 * @param {string} countryName - Full country name
 * @param {string} countryCode - Optional 2-letter ISO code
...
async function generateProxyConfig(countryName, countryCode = null) {
  try {
    ...
    if (!isoCode) {
      ...
      if (!isoCode) {
        console.warn(`No ISO code found for '${countryName}'. Defaulting to 'us'.`);
        isoCode = "us";
      }
    }

    // Generate unique session ID for this proxy
    const sessionId = generateSessionId();
    
    // Create username with country and session
    const username = `${PROXY_CONFIG.baseUsername}-${isoCode}-sessid-${sessionId}`;
    ...
    console.log(`Generated proxy config for ${countryName} (${isoCode}): ${username}`);
 ...
 * @returns {Promise<object>} Test result with IP and timing info
 */
async function testProxyConnection(proxyConfig) {
  ...
  try {
    ...
    console.log(`Proxy test successful: ${testResult.ip} (${testResult.responseTime}ms)`);
    return testResult;
 ...
 * @param {object} proxyConfig - Internal proxy configuration
 * @returns {object} Playwright-compatible proxy configuration
 */
 ...
 * @returns {Array} Array of supported country names
 */
